...
class ArteasyWooCommerceData {
    ...
    public function generate_smart_campaigns($period = '30_days') {
        ...
        // New Customer Welcome Campaign (if there are new customers in period)
        if (isset($analytics['customer_segmentation']['segments']['new_customers']) &&
            $analytics['customer_segmentation']['segments']['new_customers'] > 0) {
            $campaigns['new_customer_welcome'] = $this->generate_new_customer_welcome_campaign($analytics);
        }
        ...
        // Clearance Push Campaign (if a weakest category exists relative to top)
        if (isset($analytics['product_categories']) && !empty($analytics['product_categories'])) {
            $clearance = $this->maybe_get_clearance_category($analytics['product_categories']);
            if ($clearance) {
                $campaigns['clearance_push'] = $this->generate_clearance_push_campaign($analytics, $clearance);
            }
        }
        ...
        // Lapsed Reactivation Campaign (if there are inactive customers)
        if (isset($analytics['customer_segmentation']['segments']['inactive_customers']) &&
            $analytics['customer_segmentation']['segments']['inactive_customers'] > 0) {
            $campaigns['lapsed_reactivation'] = $this->generate_lapsed_reactivation_campaign($analytics);
        }
    ...
    private function generate_weekend_campaign($analytics) {
        $best_day = isset($analytics['selling_times']['best_day']) ? $analytics['selling_times']['best_day'] : 'Saturday';
        $weekend_performance = isset($analytics['selling_times']['weekend_performance']) ? intval($analytics['selling_times']['weekend_performance']) : 0;
        ...
        return array(
            ...
            'strategy' => array(
                'discount_coupon' => 'Create governed discount coupon (5-15% cap, single-use, no stacking)',
                'email_announcement' => 'Send announcement email to target audience (optional)',
                'targeting' => 'Target all customers or specific segments with category filters'
            ),
    ...
    private function generate_vip_campaign($analytics) {
        $vip_count = isset($analytics['customer_segmentation']['segments']['vip_customers']) ? intval($analytics['customer_segmentation']['segments']['vip_customers']) : 0;
        $avg_clv = isset($analytics['customer_segmentation']['average_clv']) ? floatval($analytics['customer_segmentation']['average_clv']) : 0;
        
        return array(
            'name' => 'VIP Customer Retention Program',
            'type' => 'vip_retention',
            'target_audience' => 'VIP customers (' . $vip_count . ' customer' . ($vip_count != 1 ? 's' : '') . ')',
            'duration' => 'Ongoing',
            'description' => "Retain and upsell your highest-value customers with average CLV of ₦" . number_format($avg_clv),
            'strategy' => array(
                'exclusive_coupon' => 'Create governed VIP-only discount coupon (5-15% cap, single-use)',
                'email_announcement' => 'Send announcement email to VIP customers (optional)',
                'targeting' => 'Restrict coupon to VIP customer emails only'
            ),
    ...
    /**
     * Generate new customer welcome campaign
     */
    private function generate_new_customer_welcome_campaign($analytics) {
        $new_count = $analytics['customer_segmentation']['segments']['new_customers'] ?? 0;
        return array(
            'name' => 'New Customer Welcome',
            'type' => 'new_customer_welcome',
            'target_audience' => 'New customers (' . intval($new_count) . ' in period)',
            'duration' => '7-14 days',
            'description' => 'Delight first-time buyers with a small, governed discount to drive the second purchase fast.',
            'strategy' => array(
                'welcome_coupon' => 'Create governed welcome discount coupon (5-15% cap, single-use)',
                'email_announcement' => 'Send welcome email to new customers (optional)',
                'targeting' => 'Target new customer segment only'
            ),
            'expected_impact' => 'Increase second-order rate by 10-20%',
            'automation_level' => 'High - Can be fully automated'
        );
    }
    ...
    private function generate_category_boost_campaign($analytics) {
        $top_category = isset($analytics['product_categories']) && !empty($analytics['product_categories']) 
            ? array_keys($analytics['product_categories'])[0] 
            : 'Featured Category';
        $category_revenue = isset($analytics['product_categories'][$top_category]['revenue']) 
            ? floatval($analytics['product_categories'][$top_category]['revenue']) 
            : 0;
        ...
        return array(
            ...
            'strategy' => array(
                'category_coupon' => 'Create governed category-specific discount coupon (5-15% cap, single-use)',
                'email_announcement' => 'Send announcement email to target audience (optional)',
                'targeting' => 'Restrict coupon to top-performing category products'
            ),
    ...
    /**
     * Compute a candidate category for clearance (weakest vs top)
     */
    private function maybe_get_clearance_category($categories) {
        if (empty($categories) || count($categories) < 1) { return false; }
        // categories is associative: name => [quantity,revenue,products]
        // Build ordered lists by revenue desc
        $ordered = $categories;
        uasort($ordered, function($a, $b){ return ($b['revenue'] ?? 0) - ($a['revenue'] ?? 0); });
        $names = array_keys($ordered);
        $top = $names[0] ?? null;
        $bottom = $names[count($names)-1] ?? null;
        if (!$top || !$bottom) { return false; }
        $top_rev = max(0, floatval($ordered[$top]['revenue'] ?? 0));
        $bottom_rev = max(0, floatval($ordered[$bottom]['revenue'] ?? 0));
        // Heuristic: bottom category has < 10% of top revenue and at least some products counted
        $bottom_products = intval($ordered[$bottom]['products'] ?? 0);
        if ($top_rev > 0 && $bottom_products > 0 && ($bottom_rev / $top_rev) < 0.10) {
            return array('name' => $bottom, 'revenue' => $bottom_rev, 'top_revenue' => $top_rev);
        }
        return false;
    }

    /**
     * Generate clearance push campaign
     */
    private function generate_clearance_push_campaign($analytics, $clearance) {
        $cat = $clearance['name'];
        $rev = $clearance['revenue'];
        return array(
            'name' => 'Clearance Push (' . $cat . ')',
            'type' => 'clearance_push',
            'target_audience' => 'All customers + price-sensitive segments',
            'duration' => '7-10 days',
            'description' => "Move slow inventory in {$cat} (₦" . number_format($rev) . ") with a governed discount and urgency.",
            'strategy' => array(
                'clearance_coupon' => 'Create governed clearance discount coupon (5-15% cap, single-use)',
                'email_announcement' => 'Send announcement email to target audience (optional)',
                'targeting' => 'Auto-targets weakest category; can refine with category filters'
            ),
            'expected_impact' => 'Liquidate stale stock while protecting margins',
            'automation_level' => 'Medium - Requires product badge/content updates'
        );
    }
    ...
    private function generate_revenue_recovery_campaign($analytics) {
        $growth_rate = isset($analytics['revenue_trends']['growth_rates']['daily_growth_rate']) 
            ? floatval($analytics['revenue_trends']['growth_rates']['daily_growth_rate']) 
            : 0;
        ...
        return array(
            ...
            'duration' => '2-3 weeks',
            'description' => "Recover declining revenue trend (currently " . number_format($growth_rate, 1) . "% growth)",
            'strategy' => array(
                'flash_coupon' => 'Create governed flash sale coupon (5-15% cap, single-use, short expiry)',
                'email_announcement' => 'Send announcement email to target audience (optional)',
                'targeting' => 'Target all customers or exclude recent purchasers'
            ),
    ...
    /**
     * Generate lapsed reactivation campaign
     */
    private function generate_lapsed_reactivation_campaign($analytics) {
        $inactive_count = $analytics['customer_segmentation']['segments']['inactive_customers'] ?? 0;
        $avg_order_value = $analytics['customer_segmentation']['average_order_value'] ?? 0;
        return array(
            'name' => 'Lapsed Customer Reactivation',
            'type' => 'lapsed_reactivation',
            'target_audience' => 'Inactive customers (' . intval($inactive_count) . ')',
            'duration' => '2-3 weeks',
            'description' => 'Win back inactive customers with a gentle offer and helpful content (based on your avg order value of ₦' . number_format($avg_order_value) . ').',
            'strategy' => array(
                'reactivation_coupon' => 'Create governed reactivation discount coupon (5-15% cap, single-use)',
                'email_announcement' => 'Send announcement email to inactive customers (optional)',
                'targeting' => 'Target inactive customer segment only'
            ),
            'expected_impact' => 'Recover 5-12% of lapsed segment',
            'automation_level' => 'High - Can be automated with timing rules'
        );
    }
    ...
    public function activate_real_campaign($campaign_id, $campaign_data) {
        ...
        switch ($campaign_data['type']) {
            ...
            case 'new_customer_welcome':
                $results = $this->activate_new_customer_welcome_campaign($campaign_data);
                break;
            case 'lapsed_reactivation':
                $results = $this->activate_lapsed_reactivation_campaign($campaign_data);
                break;
            case 'clearance_push':
                $results = $this->activate_clearance_push_campaign($campaign_data);
                break;
            default:
    ...
    /**
     * Deactivate a campaign - disable coupons and mark as inactive
     */
    public function deactivate_campaign($campaign_id) {
        if (!class_exists('WooCommerce')) {
            return array('success' => false, 'message' => 'WooCommerce not active');
        }
        
        $active_campaigns = get_option('arteasy_active_campaigns', array());
        if (!isset($active_campaigns[$campaign_id])) {
            return array('success' => false, 'message' => 'Campaign not found');
        }
        
        $campaign = $active_campaigns[$campaign_id];
        
        // Disable all coupons created by this campaign
        if (isset($campaign['results']) && is_array($campaign['results'])) {
            foreach ($campaign['results'] as $result) {
                if (isset($result['coupon_created']['id'])) {
                    $coupon_id = $result['coupon_created']['id'];
        ...
        $active_campaigns[$campaign_id]['status'] = 'inactive';
        $active_campaigns[$campaign_id]['deactivated_at'] = current_time('mysql');
        $active_campaigns[$campaign_id]['last_updated'] = current_time('mysql');
        update_option('arteasy_active_campaigns', $active_campaigns);
        
        // Update history
        $this->update_activation_history('campaign', $campaign_id, $active_campaigns[$campaign_id]);
        
        return array('success' => true, 'message' => 'Campaign deactivated successfully');
    }
    
    /**
     * Get WooCommerce product categories for dropdown
     */
    public function get_product_categories_for_select() {
        if (!class_exists('WooCommerce')) {
            return array();
        }
    ...
    private function activate_weekend_campaign($campaign_data) {
        ...
        // Create weekend discount coupon with all settings
        $coupon_code = $coupon_prefix . '_' . date('Ymd');
        
        // Process dates
        $start_date = isset($settings['campaign_start_date']) && !empty($settings['campaign_start_date']) 
            ? date('Y-m-d H:i:s', strtotime($settings['campaign_start_date'])) : null;
        $end_date = isset($settings['campaign_end_date']) && !empty($settings['campaign_end_date'])
            ? date('Y-m-d H:i:s', strtotime($settings['campaign_end_date'])) : date('Y-m-d', strtotime('+' . $expiry_days . ' days'));
        
        // Process categories (include/exclude)
        $include_cats = isset($settings['product_categories']) && is_array($settings['product_categories']) 
            ? array_filter($settings['product_categories'], function($v) { return $v !== 'all'; })
            : array();
        $exclude_cats = isset($settings['exclude_categories']) && is_array($settings['exclude_categories'])
        ...
        $coupon_id = $this->create_coupon($coupon_code, array(
            ...
            'product_categories' => !empty($include_cats) ? $include_cats : (empty($exclude_cats) ? $this->get_art_categories() : array()),
        ...
        if ($coupon_id) {
            $results['coupon_created'] = array(
                'code' => $coupon_code,
                'id' => $coupon_id,
                'discount' => $discount_amount . ($discount_type === 'percent' ? '% off' : '₦ off'),
                'expires' => date('Y-m-d', strtotime('+' . $expiry_days . ' days'))
            );
        }
        
        // Send announcement email if enabled
        if (!empty($settings['send_email'])) {
            $this->send_campaign_announcement_email($campaign_data, $coupon_code, $results);
        } else {
            // Legacy: schedule weekend email if not using new email system
    ...
    private function activate_vip_campaign($campaign_data) {
        ...
        $coupon_prefix = $settings['coupon_code'] ?? 'VIP';
        $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 20)));
        $expiry_days = intval($settings['expiry_days'] ?? 30);
        $minimum_amount = intval($settings['minimum_amount'] ?? 10000);
        
        // Create VIP exclusive coupon
        $coupon_code = $coupon_prefix . '_' . date('Ymd');
        $end_date = isset($settings['campaign_end_date']) && !empty($settings['campaign_end_date'])
            ? date('Y-m-d H:i:s', strtotime($settings['campaign_end_date']))
            : date('Y-m-d', strtotime('+' . $expiry_days . ' days'));
        
        $coupon_id = $this->create_coupon($coupon_code, array(
            'discount_type' => 'percent',
            'coupon_amount' => $discount_amount,
            'description' => 'VIP Customer Exclusive - ' . $discount_amount . '% off',
            'usage_limit' => intval($settings['usage_limit'] ?? 100),
            'usage_limit_per_user' => 1,
            'individual_use' => true,
            'expiry_date' => $end_date,
            'minimum_amount' => $minimum_amount,
    ...
    private function activate_category_campaign($campaign_data) {
        ...
        if (!empty($categories)) {
            ...
            $coupon_prefix = $settings['coupon_code'] ?? 'CATEGORY';
            $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 25)));
            $expiry_days = intval($settings['expiry_days'] ?? 14);
            $minimum_amount = intval($settings['minimum_amount'] ?? 5000);
            
            // Create category-specific coupon
            $coupon_code = $coupon_prefix . '_' . strtoupper(substr($top_category, 0, 3)) . '_' . date('Ymd');
            $end_date = isset($settings['campaign_end_date']) && !empty($settings['campaign_end_date'])
                ? date('Y-m-d H:i:s', strtotime($settings['campaign_end_date']))
                : date('Y-m-d', strtotime('+' . $expiry_days . ' days'));
            
            $cat_obj = get_term_by('name', $top_category, 'product_cat');
            $cat_ids = $cat_obj ? array($cat_obj->term_id) : array();
            
            $coupon_id = $this->create_coupon($coupon_code, array(
                'discount_type' => 'percent',
                'coupon_amount' => $discount_amount,
                'description' => "{$top_category} Special - {$discount_amount}% off",
                'usage_limit' => intval($settings['usage_limit'] ?? 500),
                'usage_limit_per_user' => 1,
                'individual_use' => true,
                'expiry_date' => $end_date,
                'minimum_amount' => $minimum_amount,
    ...
    private function activate_recovery_campaign($campaign_data) {
        ...
        $coupon_prefix = $settings['coupon_code'] ?? 'FLASH';
        $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 30)));
        $expiry_days = intval($settings['expiry_days'] ?? 3);
        $minimum_amount = intval($settings['minimum_amount'] ?? 3000);
        ...
        $coupon_id = $this->create_coupon($coupon_code, array(
            'discount_type' => 'percent',
            'coupon_amount' => $discount_amount,
            'description' => 'Flash Sale - ' . $discount_amount . '% off everything',
            'usage_limit' => intval($settings['usage_limit'] ?? 200),
            'usage_limit_per_user' => 1,
            'individual_use' => true,
            'expiry_date' => $end_date,
            'minimum_amount' => $minimum_amount
        ...
        // Send announcement email if enabled (use new generic function)
        if (!empty($settings['send_email'])) {
            $this->send_campaign_announcement_email($campaign_data, $coupon_code, $results);
        } else {
            // Legacy: send reactivation emails if not using new email system
    ...
    /**
     * Activate new customer welcome campaign
     */
    private function activate_new_customer_welcome_campaign($campaign_data) {
        $results = array();
        $settings = $campaign_data['settings'] ?? array();

        $coupon_prefix = $settings['coupon_code'] ?? 'WELCOME';
        $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 5)));
        $expiry_days = intval($settings['expiry_days'] ?? 14);
        $minimum_amount = intval($settings['minimum_amount'] ?? 5000);
        $target_customers = $settings['target_customers'] ?? 'new';
        ...
        $coupon_id = $this->create_coupon($coupon_code, array(
            'discount_type' => 'percent',
            'coupon_amount' => $discount_amount,
            'description' => 'Welcome Offer - ' . $discount_amount . '% off for new customers',
            'usage_limit' => intval($settings['usage_limit'] ?? 1000),
            'usage_limit_per_user' => 1,
            'individual_use' => true,
            'expiry_date' => $end_date,
            'minimum_amount' => $minimum_amount,
            'product_categories' => $include_cats,
            'exclude_categories' => $exclude_cats,
            'customer_email' => $this->get_target_customer_emails($target_customers)
        ));
    ...
    /**
     * Activate lapsed reactivation campaign
     */
    private function activate_lapsed_reactivation_campaign($campaign_data) {
        $results = array();
        $settings = $campaign_data['settings'] ?? array();

        $coupon_prefix = $settings['coupon_code'] ?? 'REACTIVATE';
        $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 10)));
        $expiry_days = intval($settings['expiry_days'] ?? 21);
        $minimum_amount = intval($settings['minimum_amount'] ?? 5000);
        $target_customers = $settings['target_customers'] ?? 'inactive';
        ...
        $coupon_id = $this->create_coupon($coupon_code, array(
            'discount_type' => 'percent',
            'coupon_amount' => $discount_amount,
            'description' => 'We Miss You - ' . $discount_amount . '% off reactivation offer',
            'usage_limit' => intval($settings['usage_limit'] ?? 500),
            'usage_limit_per_user' => 1,
            'individual_use' => true,
            'expiry_date' => $end_date,
    ...
    /**
     * Activate clearance push campaign
     */
    private function activate_clearance_push_campaign($campaign_data) {
        $results = array();
        $settings = $campaign_data['settings'] ?? array();

        // Get clearance category from analytics
        $categories = $this->get_product_category_performance('30_days');
        $clearance_cat = $this->maybe_get_clearance_category($categories);
        $clearance_cat_name = $clearance_cat ? $clearance_cat['name'] : 'Featured';

        $coupon_prefix = $settings['coupon_code'] ?? 'CLEARANCE';
        $discount_amount = min(15, max(5, intval($settings['discount_amount'] ?? 10)));
        $expiry_days = intval($settings['expiry_days'] ?? 10);
        $minimum_amount = intval($settings['minimum_amount'] ?? 5000);
        ...
        // Get category IDs for include
        $include_cats = isset($settings['product_categories']) && is_array($settings['product_categories'])
            ? array_filter($settings['product_categories'], function($v) { return $v !== 'all'; })
            : array();
        if (empty($include_cats) && $clearance_cat) {
            // Auto-include clearance category
            $cat_obj = get_term_by('name', $clearance_cat_name, 'product_cat');
            if ($cat_obj) $include_cats[] = $cat_obj->term_id;
        }

        $coupon_id = $this->create_coupon($coupon_code, array(
            'discount_type' => 'percent',
            'coupon_amount' => $discount_amount,
            'description' => 'Clearance Push - ' . $discount_amount . '% off ' . $clearance_cat_name,
            'usage_limit' => intval($settings['usage_limit'] ?? 500),
            'usage_limit_per_user' => 1,
            'individual_use' => true,
            'expiry_date' => $end_date,
    ...
    private function create_coupon($code, $args = array()) {
        ...
        $coupon->set_usage_limit_per_user($args['usage_limit_per_user'] ?? 1);
        $coupon->set_limit_usage_to_x_items(0);
        $coupon->set_free_shipping(false);
        $coupon->set_minimum_amount($args['minimum_amount'] ?? 0);
        
        // Individual use (no stacking) - enforced governance
        $coupon->set_individual_use($args['individual_use'] ?? true);
        
        // Start date (if provided)
        if (isset($args['start_date']) && !empty($args['start_date'])) {
            $coupon->set_date_created(strtotime($args['start_date']));
        }
        
        // Expiry date (if provided)
        if (isset($args['expiry_date']) && !empty($args['expiry_date'])) {
            $coupon->set_date_expires(strtotime($args['expiry_date']));
        } elseif (isset($args['expiry_days']) && intval($args['expiry_days']) > 0) {
            // Fallback to expiry_days
            $coupon->set_date_expires(strtotime('+' . intval($args['expiry_days']) . ' days'));
        }
        
        // Product categories (include)
        if (isset($args['product_categories']) && is_array($args['product_categories']) && !empty($args['product_categories'])) {
            if (!in_array('all', $args['product_categories'])) {
                $coupon->set_product_categories($args['product_categories']);
            }
        }
        
        // Exclude categories
        if (isset($args['exclude_categories']) && is_array($args['exclude_categories']) && !empty($args['exclude_categories'])) {
            $coupon->set_excluded_product_categories($args['exclude_categories']);
        }
        
        // Customer email restrictions
        if (isset($args['customer_email']) && is_array($args['customer_email']) && !empty($args['customer_email'])) {
    ...
    /**
     * Deactivate cart recovery system
     */
    public function deactivate_cart_recovery() {
        // Clear cron jobs
        wp_clear_scheduled_hook('arteasy_process_abandoned_carts');
        
        // Remove tracking hooks (by removing actions)
        remove_action('wp_footer', array($this, 'add_cart_abandonment_tracking'));
        remove_action('wp_ajax_arteasy_track_cart_abandonment', array($this, 'track_cart_abandonment'));
        remove_action('wp_ajax_nopriv_arteasy_track_cart_abandonment', array($this, 'track_cart_abandonment'));
        
        // Mark cart recovery as inactive
        update_option('arteasy_cart_recovery_active', false);
        ...
        return array('success' => true, 'message' => 'Cart recovery system deactivated');
    }
    ...
    public function send_recovery_email($user_id, $email_type = 'first') {
        ...
        switch ($email_type) {
            case 'first':
                ...
                $coupon_code = 'RECOVER5_' . date('Ymd');
                $message = "Hi {$user->display_name},\n\nYour art supplies are waiting! Complete your purchase and start creating.\n\nUse code {$coupon_code} for 5% off your order.";
                break;
            case 'second':
                $subject = 'Limited Time Offer: 5% Off';
                $coupon_code = 'RECOVER5_' . date('Ymd');
                $message = "Hi {$user->display_name},\n\nDon't miss out! Complete your order now and get 5% off.\n\nUse code {$coupon_code} - expires in 24 hours.";
                break;
            case 'final':
                $subject = 'Last Chance - Your Cart Expires Soon!';
                $coupon_code = 'RECOVER5_' . date('Ymd');
                $message = "Hi {$user->display_name},\n\nThis is your final chance! Your cart expires soon.\n\nUse code {$coupon_code} for 5% off - don't miss out on these art supplies.";
                break;
        }
        ...
        // Build HTML email
        // Build restore link with signed token
        $items = $this->get_order_items_summary_from_tracked($cart_data);
        $cta_url = $this->generate_restore_link($user->user_email, $items, $coupon_code);
        $message = $this->build_recovery_email_html(array(
            'customer_name' => $user->display_name ?: 'Customer',
            'coupon_code' => $coupon_code,
            'discount_label' => '5% OFF',
            'items' => $items,
            'subtotal' => floatval($cart_data['cart_total'] ?? 0),
            'cta_url' => $cta_url
        ));

        // Send email with detailed error logging (HTML)
    ...
    private function send_recovery_email_to_guest($email, $email_type, $cart_data) {
        ...
        // Build restore link with signed token
        $items = $this->get_order_items_summary_from_tracked($cart_data);
        $cta_url = $this->generate_restore_link($email, $items, $coupon_code);
        $message = $this->build_recovery_email_html(array(
    ...
    /**
     * Build branded HTML email for recovery
     */
    private function build_recovery_email_html($args) {
        $brand = get_bloginfo('name');
        $logo = get_site_icon_url() ?: '';
        $customer_name = $args['customer_name'] ?? 'Customer';
        $coupon = $args['coupon_code'] ?? '';
        $discount_label = $args['discount_label'] ?? '';
        $items = is_array($args['items'] ?? null) ? $args['items'] : array();
        $subtotal = floatval($args['subtotal'] ?? 0);
        $cta_url = $args['cta_url'] ?? get_site_url();

        $items_html = '';
        if (!empty($items)) {
            $items_html .= '<table style="width:100%; border-collapse:collapse; margin:10px 0">';
            $items_html .= '<tr><th align="left">Item</th><th align="right">Qty</th><th align="right">Price</th></tr>';
            foreach ($items as $it) {
                $items_html .= '<tr>'
                    . '<td style="padding:6px 0; border-bottom:1px solid #eee;">' . esc_html($it['name']) . '</td>'
                    . '<td style="padding:6px 0; border-bottom:1px solid #eee;" align="right">' . intval($it['qty']) . '</td>'
                    . '<td style="padding:6px 0; border-bottom:1px solid #eee;" align="right">₦' . number_format(floatval($it['price']), 2) . '</td>'
                    . '</tr>';
            }
            if ($subtotal > 0) {
                $items_html .= '<tr><td></td><td align="right"><strong>Subtotal</strong></td><td align="right"><strong>₦' . number_format($subtotal, 2) . '</strong></td></tr>';
            }
            $items_html .= '</table>';
        }

        $coupon_html = $coupon ? '<div style="background:#f5f7ff; padding:12px; border:1px dashed #6277ff; border-radius:6px; font-size:16px; letter-spacing:1px; text-align:center;"><strong>' . esc_html($coupon) . '</strong> — ' . esc_html($discount_label) . '</div>' : '';

        $logo_html = $logo ? '<img src="' . esc_url($logo) . '" alt="' . esc_attr($brand) . '" style="height:40px; margin-bottom:10px;" />' : '<div style="font-weight:700; font-size:18px; margin-bottom:10px;">' . esc_html($brand) . '</div>';

        $html = '<div style="font-family:Arial,Helvetica,sans-serif; background:#f6f7f9; padding:24px;">'
            . '<div style="max-width:560px; margin:0 auto; background:#ffffff; border:1px solid #e9ecef; border-radius:8px; overflow:hidden;">'
            . '<div style="padding:20px; border-bottom:1px solid #e9ecef;">' . $logo_html . '</div>'
            . '<div style="padding:24px;">'
            . '<h2 style="margin:0 0 8px 0; font-size:20px; color:#111">Your art supplies are waiting</h2>'
            . '<p style="margin:0 0 14px 0; color:#333">Hi ' . esc_html($customer_name) . ', complete your order to get back to creating. We saved your cart for you.</p>'
            . $coupon_html
            . ($items_html ? '<h3 style="margin:18px 0 8px 0; font-size:16px; color:#111">Items in your cart</h3>' . $items_html : '')
            . '<div style="margin:20px 0; text-align:center;">'
            . '<a href="' . esc_url($cta_url) . '" style="display:inline-block; background:#111827; color:#fff; padding:12px 18px; border-radius:6px; text-decoration:none; font-weight:600;">Return to Cart</a>'
            . '</div>'
            . '<p style="margin:8px 0 0 0; color:#666; font-size:12px;">Offer may expire soon. If you already completed your purchase, please ignore this email.</p>'
            . '</div>'
            . '<div style="padding:14px 20px; background:#fafafa; border-top:1px solid #e9ecef; color:#666; font-size:12px;">'
            . esc_html($brand) . ' • ' . esc_url(get_site_url())
            . '</div>'
            . '</div>'
            . '</div>';
    ...
    /**
     * Get simplified items summary from tracked cart snapshot
     */
    private function get_order_items_summary_from_tracked($cart_data) {
        $items = array();
        if (!empty($cart_data['cart_items']) && is_array($cart_data['cart_items'])) {
            foreach ($cart_data['cart_items'] as $ci) {
                $name = '';
                $price = 0;
                $qty = 1;
                $product_id = intval($ci['product_id'] ?? 0);
                $variation_id = intval($ci['variation_id'] ?? 0);

                // If already simplified format
                if ($product_id > 0 && isset($ci['name'])) {
                    $name = (string) $ci['name'];
                    $price = floatval($ci['price'] ?? 0);
                    $qty = max(1, intval($ci['qty'] ?? 1));
                    $items[] = array('name' => $name, 'qty' => $qty, 'price' => $price, 'product_id' => $product_id, 'variation_id' => $variation_id);
                    continue;
                }

                // WC cart line format
                if (isset($ci['data']) && is_object($ci['data'])) {
                    $name = method_exists($ci['data'], 'get_name') ? $ci['data']->get_name() : '';
                    $price = method_exists($ci['data'], 'get_price') ? floatval($ci['data']->get_price()) : 0;
                    $product_id = method_exists($ci['data'], 'get_id') ? intval($ci['data']->get_id()) : 0;
                }
                if (isset($ci['quantity'])) { $qty = intval($ci['quantity']); }
                if (isset($ci['line_total'])) { $price = floatval($ci['line_total']); }
                if ($name) {
                    $items[] = array('name' => $name, 'qty' => $qty, 'price' => $price, 'product_id' => $product_id, 'variation_id' => $variation_id);
                }
            }
    ...
    /**
     * Build items summary from a WooCommerce order
     */
    private function get_order_items_summary_from_order($order) {
        $items = array();
    ...
    /**
     * Public: Search customers by email/name/phone (for manual generator)
     */
    public function search_customers($query, $limit = 10) {
        $results = array();
        if (!function_exists('get_users')) return $results;

        $query = trim(sanitize_text_field($query));
        if ($query === '') return $results;

        // Search by email or display_name or meta (billing_phone)
        $args = array(
            'number' => $limit,
            'search' => '*' . esc_attr($query) . '*',
            'search_columns' => array('user_email', 'display_name', 'user_login'),
            'role__in' => array('customer', 'subscriber', 'shop_manager', 'administrator')
        );
        ...
        foreach ($users as $u) {
            ...
            $recent_order_id = !empty($orders) ? $orders[0]->get_id() : 0;
            ...
            $results[] = array(
                ...
                'email' => $email,
                'phone' => $phone,
                'recent_abandoned_order_id' => $recent_order_id
            );
        }
        return $results;
    }
    ...
    /**
     * Public: Search products by name/SKU (for manual generator)
     */
    public function search_products($query, $limit = 15) {
        $results = array();
        if (!class_exists('WooCommerce')) return $results;
        ...
        // Primary: search by text
        $prods = wc_get_products($args);

        // If nothing and looks like SKU, try exact SKU lookup
        if (empty($prods)) {
            $args2 = array('status' => 'publish', 'limit' => $limit, 'return' => 'objects', 'sku' => $query);
            $prods = wc_get_products($args2);
        }
    ...
    /**
     * Public: Get recent abandoned cart (order) by email
     */
    public function get_recent_abandoned_cart_by_email($email) {
        $email = sanitize_email($email);
        if (empty($email)) return array('items' => array(), 'subtotal' => 0);
        $orders = wc_get_orders(array(
            'status' => array('pending', 'on-hold'),
            'billing_email' => $email,
            'limit' => 1,
        ...
        if (empty($orders)) return array('items' => array(), 'subtotal' => 0);
        $order = $orders[0];
        return array(
            'items' => $this->get_order_items_summary_from_order($order),
            'subtotal' => floatval($order->get_total())
        );
    }

    /**
     * Generate a signed restore link for one‑click cart restoration
     */
    private function generate_restore_link($email, $items, $coupon_code = '') {
        $payload = array(
            'email' => $email,
            'items' => $items,
            'coupon' => $coupon_code,
        ...
        // Store as single-use token hash map
        $tokens = get_option('arteasy_restore_tokens', array());
        $tokens[$sig] = array('used' => false, 'created' => current_time('mysql'));
        update_option('arteasy_restore_tokens', $tokens);
    ...
    /**
     * Handle restore request: verify token, repopulate cart, apply coupon, redirect
     */
    public function handle_restore_request() {
        if (!isset($_GET['arteasy_restore']) || !isset($_GET['sig'])) {
            return;
        }
        ...
        $b64 = sanitize_text_field(wp_unslash($_GET['arteasy_restore']));
        ...
        // Check single-use
        $tokens = get_option('arteasy_restore_tokens', array());
        if (!isset($tokens[$sig]) || !is_array($tokens[$sig]) || !empty($tokens[$sig]['used'])) {
            wp_safe_redirect(wc_get_cart_url());
            exit;
        }
        ...
        // If items missing, try to fetch latest pending/on-hold order by email
        if (empty($payload['items']) && !empty($payload['email'])) {
            $orders = wc_get_orders(array(
                'status' => array('pending', 'on-hold'),
                'billing_email' => sanitize_email($payload['email']),
                'limit' => 1,
                'orderby' => 'date',
                'order' => 'DESC'
            ));
            if (!empty($orders)) {
                $payload['items'] = $this->get_order_items_summary_from_order($orders[0]);
            }
        }
        ...
        if (!empty($payload['coupon'])) {
            try { WC()->cart->apply_coupon(sanitize_text_field($payload['coupon'])); } catch (Exception $e) {}
        }

        // Mark token used
        $tokens[$sig]['used'] = true;
        $tokens[$sig]['used_at'] = current_time('mysql');
        update_option('arteasy_restore_tokens', $tokens);

        // Redirect to checkout for faster conversion
        $redirect = function_exists('wc_get_checkout_url') ? wc_get_checkout_url() : wc_get_cart_url();
    ...
    private function process_abandoned_orders($force_immediate = false) {
        ...
        foreach ($orders as $order) {
            ...
            if ($elapsed >= $first_email_delay && $recovery_sent !== 'first') {
                ...
                // Build items from order for a reliable restore token
                $items_simple = $this->get_order_items_summary_from_order($order);
                $cart_like = array(
                    'email' => $email,
                    'cart_items' => $items_simple,
                    'cart_total' => floatval($order->get_total())
                );
                
                $sent = $this->send_recovery_email_to_guest($email, 'first', $cart_like);
    ...
    /**
     * Generic campaign announcement email sender
     */
    private function send_campaign_announcement_email($campaign_data, $coupon_code, $coupon_results) {
        $settings = $campaign_data['settings'] ?? array();
        $target_type = $settings['target_customers'] ?? 'all';
        $exclude_recent_days = intval($settings['exclude_recent_days'] ?? 0);
        $include_cats = isset($settings['product_categories']) && is_array($settings['product_categories'])
            ? array_filter($settings['product_categories'], function($v) { return $v !== 'all'; })
            : array();
        $exclude_cats = isset($settings['exclude_categories']) && is_array($settings['exclude_categories'])
        ...
        // Get audience emails matching targeting using simulate function
        error_log("Arteasy Campaign: Getting audience for campaign - Target: {$target_type}, Exclude recent: {$exclude_recent_days} days");
        
        $audience = $this->simulate_campaign_audience_internal(array(
            'target_customers' => $target_type,
            'exclude_recent_days' => $exclude_recent_days,
            'product_categories' => $include_cats,
            'exclude_categories' => $exclude_cats
        ));

        $customer_emails = isset($audience['customer_emails']) ? $audience['customer_emails'] : array();
        error_log("Arteasy Campaign: Found " . count($customer_emails) . " customer emails in audience");
        
        if (empty($customer_emails)) {
            error_log("Arteasy Campaign: ❌ No audience emails to send campaign announcement");
            return;
        }

        $campaign_name = $campaign_data['name'] ?? 'Campaign';
        $discount_text = isset($coupon_results['coupon_created']['discount'])
            ? $coupon_results['coupon_created']['discount']
            : 'Special Discount';

        $subject = $campaign_name . ' - Special Offer: ' . $coupon_code;
        $message = $this->build_campaign_email_html($campaign_name, $coupon_code, $discount_text, $settings);

        $sent_count = 0;
        $error_count = 0;
        $headers = array('Content-Type: text/html; charset=UTF-8');
        
        error_log("Arteasy Campaign: Starting to send campaign announcement emails to " . count($customer_emails) . " customers");
        
        foreach ($customer_emails as $email) {
            $email_error = null;
            
            // Add filter to capture email errors
            add_filter('wp_mail_failed', function($wp_error) use (&$email_error, $email) {
                $email_error = $wp_error->get_error_message();
                error_log("Arteasy Campaign: wp_mail failed for {$email} - " . $wp_error->get_error_message());
                return $wp_error;
            });
            ...
            if ($sent) {
                ...
                $error_count++;
                error_log("Arteasy Campaign: wp_mail returned FALSE for {$email}");
                if ($email_error) {
                    error_log("Arteasy Campaign: Email error captured: {$email_error}");
                }
            }
        }

        error_log("Arteasy Campaign: ✅ Sent {$sent_count} of " . count($customer_emails) . " campaign announcement emails (Errors: {$error_count})");
    }

    /**
     * Build HTML email for campaign announcement
     */
    private function build_campaign_email_html($campaign_name, $coupon_code, $discount_text, $settings) {
        $shop_url = get_site_url();
        $coupon_url = add_query_arg('coupon_code', $coupon_code, wc_get_checkout_url());
        ...
        $html .= '<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">';
        $html .= '<h1 style="color: white; margin: 0;">' . esc_html($campaign_name) . '</h1></div>';
        $html .= '<div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">';
        $html .= '<p>Hi there,</p>';
        $html .= '<p>We have a special offer just for you!</p>';
        $html .= '<div style="background: #f8fafc; border: 2px dashed #3b82f6; padding: 20px; margin: 20px 0; text-align: center; border-radius: 8px;">';
        $html .= '<div style="font-size: 32px; font-weight: bold; color: #3b82f6; margin-bottom: 10px;">' . esc_html($coupon_code) . '</div>';
        $html .= '<div style="font-size: 24px; color: #10b981; margin-bottom: 10px;">' . esc_html($discount_text) . '</div>';
        $html .= '<p style="color: #6b7280; font-size: 14px;">Use this code at checkout</p></div>';
        $html .= '<p style="text-align: center; margin: 30px 0;">';
        $html .= '<a href="' . esc_url($coupon_url) . '" style="background: #111827; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: 600; display: inline-block;">Shop Now & Apply Code</a>';
        $html .= '</p>';
        if (isset($settings['expiry_days'])) {
            $expiry_date = date('F j, Y', strtotime('+' . $settings['expiry_days'] . ' days'));
            $html .= '<p style="color: #6b7280; font-size: 12px; text-align: center;">This offer expires ' . $expiry_date . '</p>';
        }
        $html .= '<p style="margin-top: 30px; color: #6b7280; font-size: 14px;">Happy shopping!<br>Your Art Supplies Store</p>';
        $html .= '</div></body></html>';
        return $html;
    }
    
    /**
     * Internal function to simulate campaign audience and return email list
     */
    private function simulate_campaign_audience_internal($settings) {
        $target = isset($settings['target_customers']) ? $settings['target_customers'] : 'all';
        $exclude_recent_days = isset($settings['exclude_recent_days']) ? intval($settings['exclude_recent_days']) : 0;
        
        $audience = array();
        if ($target === 'new' && method_exists($this, 'get_new_customer_emails')) {
            $audience = (array)$this->get_new_customer_emails();
        } elseif ($target === 'returning' && method_exists($this, 'get_returning_customer_emails')) {
            $audience = (array)$this->get_returning_customer_emails();
        } elseif ($target === 'vip') {
            $audience = (array)$this->get_vip_customer_emails();
        } elseif ($target === 'inactive') {
            $audience = (array)$this->get_inactive_customers();
        } else {
            // All customers
            $users = get_users(array('role' => 'customer', 'fields' => array('user_email')));
            foreach ($users as $u) {
                if (!empty($u->user_email)) {
                    $audience[] = $u->user_email;
                }
            }
            // Also get emails from orders (guest customers)
        ...
        if ($exclude_recent_days > 0 && function_exists('wc_get_orders')) {
            $recent_orders = wc_get_orders(array(
                'status' => array('wc-completed', 'completed'),
                'date_created' => date('Y-m-d', strtotime('-' . $exclude_recent_days . ' days')) . '...' . date('Y-m-d'),
                'limit' => -1,
                'return' => 'ids'
            ));
            ...
            foreach ($recent_orders as $oid) {
                $order = wc_get_order($oid);
                if ($order) {
                    $exclude_emails[] = $order->get_billing_email();
                }
            }
            $exclude_emails = array_unique(array_filter($exclude_emails));
            $audience = array_values(array_diff($audience, $exclude_emails));
        }
        
        return array(
            'audience_count' => count($audience),
            'customer_emails' => $audience,
