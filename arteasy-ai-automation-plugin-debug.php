...
define("ARTEASY_AI_VERSION", "1.5.8");
...
function arteasy_menu() {
    ...
    // Hidden submenu for Cart Recovery (accessible via main Marketing Tools)
    add_submenu_page(
        null,  // null makes it hidden from menu
        "Cart Recovery",
        "Cart Recovery",
        "manage_options",
        "arteasy-cart",
    ...
    // Development/Testing Pages (Hidden from normal view)
...
add_action("wp_ajax_arteasy_simulate_campaign", "arteasy_simulate_campaign_ajax");
add_action("wp_ajax_arteasy_export_campaign_audience", "arteasy_export_campaign_audience_ajax");
add_action("wp_ajax_arteasy_deactivate_campaign", "arteasy_deactivate_campaign_ajax");
add_action("wp_ajax_arteasy_get_campaign_categories", "arteasy_get_campaign_categories_ajax");
...
add_action("wp_ajax_arteasy_deactivate_cart_recovery", "arteasy_deactivate_cart_recovery_ajax");
...
function arteasy_generate_cart_recovery_ajax() {
    if (class_exists("ArteasyGemini")) {
        ...
        // Do NOT sanitize JSON string; it will break the structure
        ...
        if ($message) {
            ...
            $checkout_url = class_exists('WooCommerce') ? wc_get_checkout_url() : get_site_url();
            $cta_link = '<a href="' . esc_url($checkout_url) . '" style="display:inline-block; background:#111827; color:#fff; padding:12px 24px; border-radius:6px; text-decoration:none; font-weight:600; margin:10px 0;">Return to Your Cart</a>';
            $message = str_replace('[Return to Your Cart]', $cta_link, $message);
...
function arteasy_activate_campaign_ajax() {
    ...
    if (class_exists("ArteasyWooCommerceData")) {
        ...
        // Enforce coupon governance caps and sensible defaults
        $settings = isset($campaign_data['settings']) && is_array($campaign_data['settings']) ? $campaign_data['settings'] : array();
        
        // Discount governance
        $settings['discount_type'] = isset($settings['discount_type']) ? sanitize_text_field($settings['discount_type']) : 'percent';
        $settings['discount_amount'] = isset($settings['discount_amount']) ? intval($settings['discount_amount']) : 5;
        if ($settings['discount_type'] === 'percent') {
            if ($settings['discount_amount'] > 15) { $settings['discount_amount'] = 15; }
            if ($settings['discount_amount'] < 1) { $settings['discount_amount'] = 5; }
        }
        
        // Governance defaults (always enforced)
        $settings['individual_use'] = true;
        $settings['usage_limit_per_user'] = 1;
        if (!isset($settings['minimum_amount']) || intval($settings['minimum_amount']) < 5000) {
            $settings['minimum_amount'] = 5000;
        }
        
        // Process dates (if provided)
        if (isset($settings['campaign_start_date']) && !empty($settings['campaign_start_date'])) {
            $settings['campaign_start_date'] = sanitize_text_field($settings['campaign_start_date']);
        }
        if (isset($settings['campaign_end_date']) && !empty($settings['campaign_end_date'])) {
            $settings['campaign_end_date'] = sanitize_text_field($settings['campaign_end_date']);
        }
        
        // Process categories (include/exclude) - ensure arrays
        if (isset($settings['product_categories']) && is_array($settings['product_categories'])) {
            $settings['product_categories'] = array_map('intval', array_filter($settings['product_categories'], 'is_numeric'));
        } else {
            $settings['product_categories'] = array();
        }
        if (isset($settings['exclude_categories']) && is_array($settings['exclude_categories'])) {
            $settings['exclude_categories'] = array_map('intval', array_filter($settings['exclude_categories'], 'is_numeric'));
        } else {
            $settings['exclude_categories'] = array();
        }
        
        // Ensure exclude_recent_days is an integer
        if (isset($settings['exclude_recent_days'])) {
            $settings['exclude_recent_days'] = intval($settings['exclude_recent_days']);
        } else {
            $settings['exclude_recent_days'] = 0;
        }
...
// Simulate campaign audience
function arteasy_simulate_campaign_ajax() {
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'arteasy_activate_campaign')) {
        wp_send_json_error('Invalid nonce');
    }
    if (!class_exists('ArteasyWooCommerceData')) {
    ...
    $audience = array();
    if ($target === 'new' && method_exists($woo, 'get_new_customer_emails')) {
        $audience = (array)$woo->get_new_customer_emails();
    } elseif ($target === 'returning' && method_exists($woo, 'get_returning_customer_emails')) {
        $audience = (array)$woo->get_returning_customer_emails();
    } elseif ($target === 'vip') {
        $audience = (array) get_option('arteasy_vip_customers', array());
    } elseif ($target === 'inactive' && method_exists($woo, 'get_inactive_customer_emails')) {
        $audience = (array)$woo->get_inactive_customer_emails();
    } else {
        $users = get_users(array('role' => 'customer', 'fields' => array('user_email')));
        foreach ($users as $u) { if (!empty($u->user_email)) { $audience[] = $u->user_email; } }
    }
    ...
    if ($exclude_recent_days > 0 && function_exists('wc_get_orders')) {
        ...
        foreach ($recent_orders as $oid) {
            ...
            if ($order) { $exclude_emails[] = $order->get_billing_email(); }
        }
...
// Export campaign audience as CSV (base64)
function arteasy_export_campaign_audience_ajax() {
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'arteasy_activate_campaign')) {
        wp_send_json_error('Invalid nonce');
    }
    $settings_raw = isset($_POST['settings']) ? wp_unslash($_POST['settings']) : '{}';
    ...
    // Reuse simulate logic to get audience list
    $woo = new ArteasyWooCommerceData();
    $target = isset($settings['target_customers']) ? $settings['target_customers'] : 'all';
    $exclude_recent_days = isset($settings['exclude_recent_days']) ? intval($settings['exclude_recent_days']) : 0;
    $audience = array();
    ...
    $csv = "email\r\n";
    foreach ($audience as $e) { $csv .= $e . "\r\n"; }
    wp_send_json_success(array(
        'filename' => 'campaign_audience.csv',
        'content_base64' => base64_encode($csv),
        'count' => count($audience)
    ));
}

function arteasy_deactivate_campaign_ajax() {
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'arteasy_activate_campaign')) {
        wp_send_json_error('Invalid nonce');
    }
    ...
    if (class_exists('ArteasyWooCommerceData')) {
        $woo_data = new ArteasyWooCommerceData();
        $result = $woo_data->deactivate_campaign($campaign_id);
...
function arteasy_get_campaign_categories_ajax() {
    if (!wp_verify_nonce($_GET['nonce'] ?? '', 'arteasy_activate_campaign')) {
        wp_send_json_error('Invalid nonce');
    }
    
    if (class_exists('ArteasyWooCommerceData')) {
        $woo_data = new ArteasyWooCommerceData();
        $categories = $woo_data->get_product_categories_for_select();
...
function arteasy_deactivate_cart_recovery_ajax() {
    if (!wp_verify_nonce($_POST['nonce'] ?? '', 'arteasy_setup_cart_recovery')) {
        wp_send_json_error('Invalid nonce');
    }
    ...
    if (class_exists("ArteasyWooCommerceData")) {
        $woo_data = new ArteasyWooCommerceData();
        $result = $woo_data->deactivate_cart_recovery();
